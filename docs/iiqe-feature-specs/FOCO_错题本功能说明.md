...
2. **预估成本**：这一块大概有多少错题、掌握难度分布如何（还差 1 次/2 次/反复错）。  
...
#### 4.3.3 复活提示（居中轻提示） + 反复错红条（持续提示）

当已归档题（在「已掌握错题」中/其他章节/分重点刷题的刷题页中）再次答错：

- **瞬时反馈**：居中轻提示 `该题已复活回错题本`（1.2s 左右，`pointer-events: none`，不遮挡题干阅读流）。
- **持续反馈**：该题进入 `isRepeatWrong=true` 后，在掌握进度卡片下方出现**红色提示底栏**：  
`反复错，建议深入看解析/找老师答疑`

**设计意图**：

- 复活是状态变化，适合“toast/轻提示”告知事实；  
- “反复错”是学习建议，必须在当前题的掌握区域**持续存在**，避免一闪而过。
...
| 反复错        | `isRepeatWrong = true`    | 归档后再次答错并复活的题目          |
| 掌握进度 `···` | `consecutiveCorrect` 进度指示 | 三点对应 0 / 1 / 2 / 3 次答对 |
|            |                           |                        |
