...
|------|----------------------|----------------------------|
| **含义** | 曾归档后又丢了（或累计错太多被系统视为需巩固的复活态） | 这道题历史上错得太多次 |
| **何时写入** | ① 已掌握后再答错；② 本次答错后 `wrongCount > 5` 时也会置 `true` | 每次作答后**计算**：`wrongCount > 5` |
...
- **反复错 ⇒ 一定是复活题**（错 >5 次时答错会把 `revived` 置 true）。
- **复活题 ⇏ 一定是反复错**（刚从已掌握复活、累计才错 1～5 次时：`revived=true`，`repeatedWrong=false`）。
...
| 展示 | 触发（代码） | 位置 | 样式 |
|------|-------------|------|------|
| **复活题** | 未归档且 `(revived \|\| repeatedWrong)` | 掌握进度标题行右对齐 | 橙色胶囊「复活题」 |
...
**设计意图**：复活 = 身份（角标）；反复错 = 风险（红条）；二者可并存展示，但不是同一标记。
