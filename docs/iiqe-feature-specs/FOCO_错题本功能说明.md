...
- **文案**：`归档到已掌握`（归档中显示 `归档中...`）。
- **交互**：点击后弹出归档确认弹窗；确认后底栏变**浅灰**、文案「已归档」，题目 `archived` 并从「待复习」计入「已掌握错题」；居中轻提示后可进下一题。
...
- **掌握进度区展示**：**仅错题本所属刷题页**（顶栏「错题本」、`fromErrors` 且带 `sectionId`）在出解析后显示「掌握进度」卡片及底栏；章节或分重点刷题页**不展示**该区，但后台仍累计进度/复活/反复错。
- **叠层**：仅两层（掌握进度卡 + 一条底栏）；有底栏时卡片仅上圆角，不用延伸阴影填缝；底栏文案在可见带（44−14px）内垂直居中。
- **复活轻提示**：非错题本页答错触发复活时，仍可出现居中轻提示「该题已复活回错题本」（约 1.2s），不展示掌握进度卡片。

#### 4.3.4 复活题（标题角标）+ 反复错（红色底栏）

**关系**：反复错一定是复活题；复活题未必已达反复错阈值。

| 展示 | 触发 | 说明 |
|------|------|------|
| **复活题** | `revived` 且未归档 | 与「掌握进度」标题同行右对齐橙色角标「复活题」 |
| **可归档** | 可归档且未归档 | 绿色可点击底栏（与红条互斥，优先绿条） |
| **反复错** | `wrongCount > 5` 且不可归档 | 红色底栏 |
